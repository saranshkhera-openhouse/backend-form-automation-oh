...
// Pills with "None/No Items" deselect logic
...
// ══════ MULTI-SELECT (Checkbox list, NOT dropdown) ══════
function makeMultiSelect(container,name,options){
  const selected=new Set();
  container.innerHTML=`<div class="msel"><input type="text" class="msel-search" placeholder="Search documents..."><div class="msel-opts"></div><div class="msel-count">0 selected</div></div>`;
...
// ══════ IMAGE UPLOAD (Cloudinary) — NO capture attribute ══════
...
// ══════ SINGLE IMAGE UPLOAD — NO capture attribute ══════
...
// ══════ CONTACT DIGIT VALIDATION (realtime) ══════
function setupContactValidation(inputId,errId){
  ...
  inp.addEventListener('input',()=>{
    const len=inp.value.replace(/\D/g,'').length;
    if(len>0&&len<10){err.textContent=`${10-len} more digits needed`;err.classList.add('show')}
...
// ══════ AUTO-CLEAR VALIDATION RED BOX ══════
document.addEventListener('input',e=>{const fg=e.target.closest('.fg');if(fg&&fg.classList.contains('invalid')){if(e.target.value.trim())fg.classList.remove('invalid')}});
document.addEventListener('change',e=>{const fg=e.target.closest('.fg');if(fg&&fg.classList.contains('invalid')){if(e.target.value)fg.classList.remove('invalid')}});
...
function fillUidSelect(sel,uids){
  ...
  uids.forEach(u=>{
    const parts=[u.society_name,u.tower_no?`${u.tower_no}-`:'',u.unit_no].filter(Boolean).join('');
...
function configDisplay(config,extraArea){
  if(!config)return '—';
  if(!config.includes('.5'))return config;
