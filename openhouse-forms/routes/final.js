...
module.exports=function(pool){
  router.get('/prefill/:uid',async(req,res)=>{
    try{const{rows}=await pool.query('SELECT * FROM properties WHERE uid=$1',[req.params.uid]);
      ...
      const p=rows[0];if(!p.token_deal_submitted_at&&!p.token_submitted_at)return res.status(400).json({error:'Token Deal form must be submitted first'});
      res.json(p)}catch(e){res.status(500).json({error:e.message})}
  });
  router.get('/uids',async(_,res)=>{
    try{const{rows}=await pool.query(`SELECT uid,city,society_name,unit_no,owner_broker_name,token_deal_submitted_at,final_submitted_at
  ...
  router.post('/submit',async(req,res)=>{
    try{
      ...
      await pool.query(`UPDATE properties SET
        ...
        token_transfer_date=$21,neft_reference=$22,final_submitted_at=NOW(),updated_at=NOW()
        WHERE uid=$23`,
  ...
  router.get('/pdf/:uid',async(req,res)=>{
    try{const{rows}=await pool.query('SELECT * FROM properties WHERE uid=$1',[req.params.uid]);
      ...
      res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`inline; filename=FinalToken_${rows[0].uid}.pdf`);res.send(buf);
