// const PDFDocument = require('pdfkit');

// function generateTokenPDF(data) {
//   return new Promise((resolve, reject) => {
//     try {
//       const doc = new PDFDocument({
//         margin: 50,
//         size: 'A4',
//         bufferPages: true,
//         info: {
//           Title: `Token Agreement - ${data.uid}`,
//           Author: 'Openhouse',
//         },
//       });

//       const buffers = [];
//       doc.on('data', (chunk) => buffers.push(chunk));
//       doc.on('end', () => resolve(Buffer.concat(buffers)));

//       const pageW = doc.page.width;
//       const contentW = pageW - 100;

//       // ── Header ──
//       doc.rect(0, 0, pageW, 90).fill('#111827');
//       doc.fill('#ffffff').fontSize(22).font('Helvetica-Bold').text('OPENHOUSE', 50, 25);
//       doc.fontSize(11).font('Helvetica').text('Token Agreement', 50, 52);
//       doc.fontSize(9).fillColor('#94a3b8')
//          .text(`UID: ${data.uid}  |  Generated: ${new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}`, 50, 68);

//       let y = 110;

//       const section = (title, fields) => {
//         if (y > 720) { doc.addPage(); y = 50; }

//         doc.rect(50, y, contentW, 26).fill('#f1f5f9');
//         doc.fill('#111827').fontSize(10).font('Helvetica-Bold').text(title, 60, y + 7);
//         y += 34;

//         fields.forEach(([label, value]) => {
//           if (y > 750) { doc.addPage(); y = 50; }
//           doc.fill('#64748b').fontSize(8.5).font('Helvetica').text(label, 60, y, { width: 190 });
//           doc.fill('#111827').fontSize(9.5).font('Helvetica-Bold')
//              .text(String(value ?? '—'), 260, y, { width: 280 });
//           y += 20;
//         });
//         y += 8;
//       };

//       section('PROPERTY IDENTIFICATION', [
//         ['UID', data.uid],
//         ['City', data.city],
//         ['Locality', data.locality],
//         ['Society Name', data.society_name],
//         ['Unit No.', data.unit_no],
//         ['Floor', data.floor],
//         ['Configuration', data.configuration],
//         ['Size (sqft)', data.area_sqft],
//       ]);

//       section('OWNERSHIP', [
//         ['Owner / Broker Name', data.owner_broker_name],
//         ['Co-owner', data.co_owner],
//         ['Registry Status', data.registry_status],
//         ['Occupancy Status', data.occupancy_status],
//         ['Inclusions', data.inclusions],
//       ]);

//       section('SALE TERMS', [
//         ['Guaranteed Sale Price (₹ Lakhs)', data.guaranteed_sale_price],
//         ['Performance Guarantee (₹)', data.performance_guarantee],
//         ['Full Registry Value Willingness', data.full_registry_willingness],
//         ['Initial Period (Days)', data.initial_period],
//         ['Grace Period (Days)', data.grace_period],
//         ['Grace Period Alignment', data.grace_period_alignment],
//       ]);

//       section('LOAN DETAILS', [
//         ['Outstanding Loan Amount (₹)', data.outstanding_loan],
//         ['Bank Name (Loan A/c)', data.bank_name_loan],
//         ['Loan Account Number', data.loan_account_number],
//         ['Willing to Pay Loan', data.loan_pay_willingness],
//       ]);

//       section('DOCUMENTS', [
//         ['Papers Available', data.papers_available],
//         ['Documents', data.documents_available],
//       ]);

//       section('PAYMENT', [
//         ['Bank A/C Number', data.bank_account_number],
//         ['Bank Name', data.bank_name],
//         ['IFSC Code', data.ifsc_code],
//         ['Token Paid (₹)', data.token_paid],
//         ['Transfer Date', data.token_transfer_date ? new Date(data.token_transfer_date).toLocaleDateString('en-IN') : null],
//         ['NEFT Reference', data.neft_reference],
//       ]);

//       if (data.token_remarks) {
//         section('REMARKS', [['Remarks', data.token_remarks]]);
//       }

//       const pageCount = doc.bufferedPageRange().count;
//       for (let i = 0; i < pageCount; i++) {
//         doc.switchToPage(i);
//         doc.fill('#94a3b8').fontSize(7).font('Helvetica')
//            .text(
//              `Openhouse Token Agreement  •  ${data.uid}  •  Page ${i + 1} of ${pageCount}`,
//              50, doc.page.height - 30,
//              { align: 'center', width: contentW }
//            );
//       }

//       doc.end();
//     } catch (err) {
//       reject(err);
//     }
//   });
// }

// module.exports = { generateTokenPDF };


const PDFDocument = require('pdfkit');
const fs = require('fs');
const path = require('path');

function safe(val) {
  if (val === null || val === undefined) return '';
  return String(val).trim();
}

function splitOwners(name) {
  if (!name) return { owner: '', coOwner: '' };
  const parts = name.split(/,| and | & /i);
  return {
    owner: parts[0]?.trim() || '',
    coOwner: parts.slice(1).join(', ').trim() || ''
  };
}

async function generateTokenPDF(data, outputPath) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        size: 'A4',
        margins: { top: 36, bottom: 36, left: 36, right: 36 } // 0.5 inch
      });

      const stream = fs.createWriteStream(outputPath);
      doc.pipe(stream);

      const pageWidth = doc.page.width;
      const margin = 36;
      const contentWidth = pageWidth - margin * 2;

      // ───────── HEADER (LOGO RIGHT) ─────────
      if (data.logoPath && fs.existsSync(data.logoPath)) {
        doc.image(data.logoPath, pageWidth - 110, 20, { width: 80 });
      }

      doc.moveDown(2);

      // ───────── TITLE ─────────
      doc
        .font('Helvetica-Bold')
        .fontSize(16)
        .text('Token Payment Receipt', {
          align: 'center',
          underline: true
        });

      doc.moveDown(1);

      // ───────── INTRO ─────────
      const { owner, coOwner } = splitOwners(data.seller_name);

      doc
        .font('Helvetica')
        .fontSize(11)
        .text('Dear ', { continued: true })
        .font('Helvetica-Bold')
        .text(owner);

      doc.moveDown(0.3);

      doc
        .font('Helvetica')
        .fontSize(11)
        .text('Please find the major details of the transaction below:');

      doc.moveDown(1);

      // ───────── TABLE SETUP ─────────
      const col1Width = 200;
      const col2Width = contentWidth - col1Width;
      let y = doc.y;

      const BLUE = '#BDD7EE';
      const BROWN = '#FCE4D6';

      const rows = [
        ['City', data.city, BLUE],
        ['Society Name', data.society_name, BLUE],
        ['Owner Name', owner, BLUE],
        ['Co-owner (if any)', coOwner, BLUE],
        ['Unit No.', data.unit_no, BLUE],
        ['Floor', data.floor, BLUE],
        ['Configuration', data.configuration, BLUE],
        ['Size (in sqft)', data.size_sqft, BLUE],
        ['Registry Status', data.registry_status, BLUE],
        ['Occupancy Status', data.occupancy_status, BLUE],

        ['Guaranteed Sale Price (in INR Lakhs)', data.agreed_price, BROWN],
        ['Performance Guarantee (in INR)', data.performance_guarantee, BROWN],
        [
          'Beta requirement (Mention range in %age)',
          `${safe(data.beta_min)} - ${safe(data.beta_max)}`,
          BROWN
        ],
        ['Initial Period (in Days)', data.initial_period, BROWN],
        ['Grace Period (in Days)', data.grace_period, BROWN],
        ['Grace Period Alignment', data.maintenance_alignment, BROWN],

        ['Outstanding Loan Amount (if any)', data.outstanding_loan, BLUE],
        ['Name of the Bank (with Loan A/c)', data.bank_detail_loan, BLUE],
        ['Loan Account Number (LAN)', data.lan_number, BLUE],
        ['Willingness to pay Loan on his own? (Y/N)', data.loan_closure_plan, BLUE],

        ['Seller confirms required property papers available?', 'YES', BLUE],

        ['Bank A/C number', data.bank_account, BROWN],
        ['Bank Name', data.bank_name, BROWN],
        ['IFSC Code', data.ifsc_code, BROWN],
        ['Token Paid', data.token_paid, BROWN],
        ['Token Transfer Date', data.token_date, BROWN],
        ['NEFT Reference No.', data.neft_reference, BROWN],

        ['Remarks (if any)', data.remarks, BLUE],
      ];

      rows.forEach(([label, value, color]) => {
        const rowHeight = 22;

        // Background Left Cell
        doc
          .rect(margin, y, col1Width, rowHeight)
          .fill(color);

        // Background Right Cell
        doc
          .rect(margin + col1Width, y, col2Width, rowHeight)
          .fill('#FFFFFF');

        // Borders
        doc
          .rect(margin, y, col1Width + col2Width, rowHeight)
          .stroke('#000000');

        // Text Left
        doc
          .fill('#000000')
          .font('Helvetica')
          .fontSize(10)
          .text(label, margin + 5, y + 6, {
            width: col1Width - 10
          });

        // Text Right
        doc
          .font('Helvetica')
          .fontSize(10)
          .text(safe(value), margin + col1Width + 5, y + 6, {
            width: col2Width - 10
          });

        y += rowHeight;
      });

      // ───────── FOOTER ─────────
      doc.fontSize(10);
      doc.moveDown(2);

      doc
        .font('Helvetica-Bold')
        .text('Avano Technologies Private Limited', {
          align: 'center'
        });

      doc
        .font('Helvetica')
        .fontSize(9)
        .text(
          'CIN: U68200HR2024PTC123116\nVentureX, Unit No. 202 & 202A, Silverton Tower, Sector 50,\nGolf Course Extension Road, Gurugram, Haryana 122018, India',
          {
            align: 'center'
          }
        );

      doc.end();

      stream.on('finish', () => resolve());
    } catch (err) {
      reject(err);
    }
  });
}

module.exports = { generateTokenPDF };