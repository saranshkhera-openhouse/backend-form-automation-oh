...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  body{font-family:'DM Sans',sans-serif;background:var(--bg);color:var(--ink);padding:14px 16px 16px;font-size:12px}
  .page{max-width:680px;margin:0 auto}
  .header{display:flex;align-items:center;justify-content:space-between;padding-bottom:16px;margin-bottom:16px;border-bottom:1.5px solid var(--border)}
  ...
  .greeting-strip{background:var(--ink);border-radius:8px;padding:18px 20px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
  .greeting-left .hi{font-size:20px;font-weight:400;color:var(--white)}
  .greeting-left .sub{font-size:11px;color:rgba(255,255,255,.6);margin-top:4px;font-weight:300}
  ...
  .price-val{font-size:26px;font-weight:600;color:var(--white);line-height:1}
  .guarantee-pill{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);border-radius:6px;padding:10px 14px;text-align:center;flex-shrink:0}
  .guarantee-pill .gv{font-size:16px;font-weight:600;color:var(--gold-light)}
  .guarantee-pill .gl{font-size:9px;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.4);margin-top:2px}
  .section-label{font-size:9.5px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);font-weight:500;margin:22px 0 10px;display:flex;align-items:center;gap:8px}
  .section-label::after{content:'';flex:1;height:1px;background:var(--border)}
  .field-grid{display:grid;gap:10px}.field-grid.col2{grid-template-columns:1fr 1fr}.field-grid.col3{grid-template-columns:1fr 1fr 1fr}.field-grid.col4{grid-template-columns:1fr 1fr 1fr 1fr}
  .field{background:var(--white);border:1px solid var(--border);border-radius:6px;padding:12px 14px}
  .field .f-label{font-size:8.5px;letter-spacing:.09em;text-transform:uppercase;color:var(--muted);font-weight:500;margin-bottom:4px}
  .field .f-value{font-size:12.5px;font-weight:500;color:var(--ink);line-height:1.2}
  .field .f-value.mono{font-family:monospace;font-size:11.5px;letter-spacing:.04em}
  .field .f-value.empty{color:#bbb;font-weight:300;font-size:11.5px;font-style:italic}
  ...
  .token-strip{background:var(--green-light);border:1.5px solid #b8d9c4;border-radius:7px;padding:14px 18px;display:flex;align-items:center;gap:12px}
  .token-icon{width:30px;height:30px;background:var(--green);border-radius:50%;display:grid;place-items:center;flex-shrink:0}
  .token-icon svg{width:15px;height:15px;stroke:white;fill:none;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round}
  .token-info{flex:1}.token-info .t-title{font-size:12.5px;font-weight:500;color:var(--green)}
  .token-info .t-ref{font-size:11.5px;color:#4a7a5d;font-family:monospace;margin-top:2px}
  .token-date{text-align:right;flex-shrink:0}
  .token-date .td-label{font-size:8.5px;text-transform:uppercase;letter-spacing:.09em;color:#4a7a5d}
  .token-date .td-val{font-size:14px;font-weight:600;color:var(--green);margin-top:2px}
  .doc-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
  .doc-item{background:var(--white);border:1px solid var(--border);border-radius:6px;padding:12px 14px;display:flex;align-items:center;gap:10px;font-size:12px}
  ...
  .terms-wrap{background:var(--cream);border:1px solid var(--border);border-radius:8px;padding:20px 24px}
  .terms-wrap h4{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);font-weight:500;margin-bottom:14px}
  .terms-list{list-style:none;display:flex;flex-direction:column;gap:12px}
  .terms-list li{display:flex;gap:10px;font-size:12.5px;color:#4a4035;line-height:1.6;font-weight:300}
  .terms-list li::before{content:'—';color:var(--gold);flex-shrink:0}
  .footer{margin-top:24px;padding-top:16px;border-top:1px solid var(--border);display:flex;align-items:flex-start;justify-content:space-between;gap:16px}
  .footer-brand{font-size:13px;font-weight:600;margin-bottom:4px}
  .footer-cin{font-size:10px;color:var(--muted);font-weight:300;line-height:1.6}
...
<body><div class="page">
  ...
  <div class="field-grid col3" style="margin-top:10px">
  ...
  ${hdDate?`<div class="field-grid col2" style="margin-top:10px"><div class="field"><div class="f-label">Key Handover Date</div><div class="f-value">${hdDate}</div></div><div class="field"></div></div>`:''}
  ...
  ${(p.initial_period||p.grace_period)?`<div class="field-grid col2" style="margin-top:10px">
  ...
  ${missingDocs.length?`<div class="section-label" style="margin-top:12px">Documents Missing</div>
  ...
  <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:12px 14px;font-size:11px;color:#374151;line-height:1.4;white-space:pre-wrap;max-height:80px;overflow:hidden">${esc(p.token_remarks_printed)}</div>`:''}
